...
def iter_probe_specific_model(prefix, budget, alias_prefix):

    path = config.zmap_result_path + 'prefix_hit_alias_info.txt'
    df_prefix_hit_alias = pd.read_csv(path, index=False)
    ...
    while hit_rate_no_alias > 0.0:
        ...
        hit_rate, hit_rate_no_alias, alias_prefix_list, num_init_probe_address = test_specific_model(prefix, num_active * growth_factor)
        iter_info_record.append([prefix, hit_rate, hit_rate_no_alias, alias_prefix_list, num_init_probe_address])

    df_all_active_address.to_csv
    



def iter_probe_all_model(prefix, budget):
    path = config.zmap_result_path + 'prefix_hit_alias_info.txt'
